...
from ..agency.storage import AgencyStorage
...
import aries_cloudagency.wallet.util as util
import aries_cloudagency.agency.wallet as agency_wallet
...
class PackWireFormat(BaseWireFormat):
    ...
    async def unpack(
        ...
        try:
            # Look up agency to find right wallet for recipient

            message_json = json.loads(message_body)
            decoded_protected_str = util.b64_to_str(message_json['protected'])
            decoded_protected_json = json.loads(decoded_protected_str)

            recipients = decoded_protected_json['recipients']
